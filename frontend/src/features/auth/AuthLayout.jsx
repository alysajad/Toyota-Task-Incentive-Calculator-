...
function VisualPanel() {
  return (
    <section className="relative order-first h-44 w-full overflow-hidden bg-slate-950 sm:h-56 md:h-64 lg:order-none lg:h-auto lg:min-h-[720px]">
      ...
      <div className="absolute bottom-9 left-14 hidden w-[23rem] rounded-2xl border border-white/70 bg-surface/80 p-4 shadow-2xl shadow-slate-950/10 backdrop-blur-xl lg:block">
        <div className="flex items-center justify-between gap-3">
          <div>
            <div className="text-xs font-bold uppercase text-muted">Access flow</div>
            <div className="mt-1 text-lg font-extrabold text-content">Verified before payout data</div>
          </div>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {workflowSteps.map(({ icon: Icon, label, value }) => (
            <div key={label} className="rounded-xl border border-line/80 bg-surface/75 px-3 py-3">
              <Icon className="h-5 w-5" />
              <div className="mt-3 text-xs font-extrabold text-content">{label}</div>
              <div className="mt-0.5 text-[11px] font-medium leading-4 text-muted">{value}</div>
            </div>
          ))}
        </div>
      </div>
...
export function AuthLayout({ children }) {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-surface-inset px-4 py-5 sm:px-6 lg:px-10">
      <main className="mx-auto flex min-h-[calc(100vh-2.5rem)] max-w-7xl items-center justify-center">
        <motion.div
          initial={{ opacity: 0, y: 18, scale: 0.985 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          transition={{ duration: 0.42, ease: 'easeOut' }}
          className="relative flex w-full flex-col overflow-hidden rounded-[2rem] border border-line bg-surface shadow-[0_35px_90px_-45px_rgba(15,23,42,0.45)] lg:grid lg:min-h-[720px] lg:grid-cols-[0.92fr_1.08fr]"
        >
          <header className="absolute inset-x-0 top-0 z-20 flex h-24 items-center justify-between px-6 sm:px-10 lg:px-12">
            ...
            <div className="flex items-center gap-3">
                ...
                {navItems.map((item) => (
                  <Link
                    ...
                    className={cn(
                      ...
                      location.pathname === item.to ? 'text-content' : 'text-subtle',
                    )}
              ...
              <ThemeToggle className="bg-surface/70 backdrop-blur-sm" />
            </div>
          </header>
            ...
            <div className="w-full max-w-md">
              ...
              <div className="mt-7 grid gap-2 sm:grid-cols-3">
                {highlights.map(({ icon: Icon, label }) => (
                  <div key={label} className="flex items-center gap-2 rounded-xl border border-line bg-surface-soft px-3 py-2.5">
                    <Icon className="h-4 w-4 shrink-0 text-toyota" />
                    <span className="text-[11px] font-bold leading-4 text-muted">{label}</span>
                  </div>
                ))}
              </div>
            </div>
